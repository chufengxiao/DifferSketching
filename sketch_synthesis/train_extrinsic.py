...
class MLP:
    def train(self,save_model_path,NP='N'):
        ...
        ## ---------------- load data
        x = np.loadtxt('./data/extrinsic/in_curves_%d_%s.txt'%(control_num,NP))
        y = np.loadtxt('./data/extrinsic/out_%d_%s.txt'%(control_num,NP))
        ...
        ## ---------------- Split dataset
        np.random.seed(233)
        x_train, x_test, y_train, y_test = train_test_split(x_input,y,test_size = 0.01)
...
if __name__ == '__main__':
    ...
    save_model_path = os.path.join(save_folder,f"{NP}_mlp_extrinsic.sav")
